...
use super::super::super::no_session::{NoSession, SessionData};
use super::super::super::State as SuperState;
...
trait SessionState<R: radio::AsyncPhyRxTx + Timings> {
...
macro_rules! into_state {
    ($($from:tt),*) => {
    $(
        impl<'a, R: radio::AsyncPhyRxTx + Timings, C: CryptoFactory + Default> From<$from<'a, R>> for Device<'a, R, C>
        {
        ...
        impl<'a, R: radio::AsyncPhyRxTx + Timings> SessionState<R> for $from<'a, R> {
            fn get_session(&self) -> &SessionData {
                &self.session
            }
        }

        impl<'a, R: radio::AsyncPhyRxTx + Timings> CommonState<'a, R> for $from<'a, R> {
...
impl<R> From<Error> for super::super::super::Error<R>
where
    R: radio::AsyncPhyRxTx,
{
    fn from(error: Error) -> super::super::super::Error<R> {
        super::super::super::Error::Session(error)
    }
}
    ...
    R: radio::AsyncPhyRxTx + Timings,
{
    ...
    pub async fn handle_event<C: CryptoFactory + Default>(
        ...
        Device<'a, R, C>,
        Result<Response, super::super::super::Error<R>>,
    ) {
        match self {
                                ...
                                if session.devaddr() == &encrypted_data.fhdr().dev_addr() {
                                        ...
                                        && (fcnt > session.fcnt_down || fcnt == 0)
                                    {
                                        ...
                                        self.shared.downlink =
                                            Some(super::super::Downlink::Data(decrypted));
...
fn data_rxwindow1_timeout<R: radio::AsyncPhyRxTx + Timings, C: CryptoFactory + Default>(
    state: Session<'_, R>,
    confirmed: bool,
